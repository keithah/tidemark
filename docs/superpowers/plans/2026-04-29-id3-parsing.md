...
> Status: Implemented. This file is a historical implementation plan, not current build guidance. Current behavior is documented in `docs/reference.md`; `ParseFromMPEGTS` now returns grouped timed-ID3 events as `[][]Tag` so segment decoding can emit one marker per event.
...
**Architecture:** Two additions to `internal/id3/id3.go` — a one-line change to `parseFrame` to handle all `T***` frames, and a new `ParseFromMPEGTS` function that walks 188-byte TS packets, reassembles PES payloads per PID, strips PES headers, and delegates to the existing `Parse`. The final implementation returns grouped tag events.
